...
def parse_system_name(dataset_name):
    ...
    if name.startswith("wt"):
        return "wt"
    if name.startswith("sdt3"):
        return "sdt3"
    if name.startswith("sdt5"):
        return "sdt5"
    if name.startswith("sdt8"):
        return "sdt8"
    return name
...
for mutant in ["sdt3", "sdt5", "sdt8"]:
    ...
    fig.savefig(
        OUTPUT_DIR / f"TOGT_SX1_residue_type_wt_vs_{mutant}.png",
        dpi=DPI,
        bbox_inches="tight",
    )
    plt.close(fig)
...
for mutant in ["sdt3", "sdt5", "sdt8"]:
    for suffix, stem, out_name in [
        ("", "Fig5B", f"TOGT_Fig5B_delta_kd_vs_delta_sasa_{mutant}.png"),
        ("-filtered", "SX2", f"TOGT_SX2_delta_kd_vs_delta_sasa_filtered_{mutant}.png"),
    ]:
        ...
        ax.plot(reg_line_x, reg_line_y, color=SDT5_COLOR, linewidth=1.2, linestyle="--", zorder=1.5)
...
legend_handles = []
for system in ["wt", "sdt3", "sdt5", "sdt8"]:
...
fig.savefig(OUTPUT_DIR / "TOGT_SX3_RMSD_timecourse.png", dpi=DPI, bbox_inches="tight")
plt.close(fig)
...
rmsf_df = rmsf_df.loc[rmsf_df["x_value"].between(1, 476)].copy()
...
ax.tick_params(axis="both", labelsize=TICK_SIZE)
ax.set_xlim(1, 476)
ax.set_xticks([1, 100, 200, 300, 400, 476])
ax.set_ylim(0, 0.40)
ax.legend(handles=legend_handles, frameon=False, fontsize=LEGEND_SIZE, ncol=4, loc="upper left")

fig.subplots_adjust(left=0.10, right=0.98, bottom=0.18, top=0.96)
fig.savefig(OUTPUT_DIR / "TOGT_SX8_RMSF_mean_shadow.png", dpi=DPI, bbox_inches="tight")
plt.close(fig)
...
FIG5C_SYSTEMS = ["wt", "sdt3", "sdt5", "sdt8"]
...
fig5c_xy_stats_df.to_csv(
    OUTPUT_DIR / "TOGT_Fig5C_xy_mean_variance_summary.csv",
    index=False,
    encoding="utf-8-sig",
)
...
def format_fig5c_axes(ax):
    ...
    """
    ax.set_xlabel(r"$d$(His16-NE2, ESC-O7) (nm)", fontsize=LABEL_SIZE)
...
def plot_single_system_scatter(system):
    ...
    fig.savefig(
        OUTPUT_DIR / f"TOGT_Fig5C_deltaG_bind_vs_distance_{system}.png",
        dpi=DPI,
        bbox_inches="tight",
    )
    plt.close(fig)
...
plot_overlay_scatter(
    ...
    output_name="TOGT_Fig5C_deltaG_bind_vs_distance_overlay.png",
    alpha=0.35,
)


# -------------------------
# 3) wt vs each mutant overlays
# -------------------------
for mutant in ["sdt3", "sdt5", "sdt8"]:
    plot_overlay_scatter(
        system_list=["wt", mutant],
        output_name=f"TOGT_Fig5C_deltaG_bind_vs_distance_wt_vs_{mutant}_overlay.png",
        alpha=0.35,
    )
...
hb_summary_df.to_csv(
    OUTPUT_DIR / "TOGT_Fig5C_hbond_distance_fraction_summary.csv",
    index=False,
    encoding="utf-8-sig",
)
...
for system in ["wt", "sdt3", "sdt5", "sdt8"]:
    ...
    pooled_values = []
    for rep in [1, 2, 3, 4]:
        sub = rg_df.loc[(rg_df["system_group"] == system) & (rg_df["replicate_id"] == rep), "y_value"].values
...
fig, ax = plt.subplots(figsize=FIG_VIOLIN_16)
parts = ax.violinplot(rg_groups, positions=rg_positions, widths=0.78, showmeans=False, showmedians=False, showextrema=False)
...
for i, pos in enumerate(rg_positions):
    ...
    ax.vlines(pos, rg_means[i] - rg_stds[i], rg_means[i] + rg_stds[i], color="black", linewidth=1.1, zorder=3)
    ax.text(pos, 2.1525, f"{rg_means[i]:.3f}\n+/- {rg_stds[i]:.3f}", ha="center", va="bottom", fontsize=7.6, color="#333333", linespacing=0.95)

# ax.set_xlabel("Replicate", fontsize=LABEL_SIZE)
...
for center, system in zip(rg_system_centers, ["wt", "sdt3", "sdt5", "sdt8"]):
    ax.text(center, -0.06, system, transform=ax.get_xaxis_transform(), ha="center", va="top", fontsize=LABEL_SIZE)

legend_handles = [
    Patch(facecolor=SYSTEM_COLORS[system], edgecolor="black", alpha=0.75, label=system)
    for system in ["wt", "sdt3", "sdt5", "sdt8"]
]
legend_handles.append(Line2D([0], [0], marker="o", color="black", markerfacecolor="white", linewidth=1.1, label="Mean +/- SD"))
...
fig.savefig(OUTPUT_DIR / "TOGT_SX4_Rg_violin_no_significance.png", dpi=DPI, bbox_inches="tight")

# replicate-level summary for significance testing
rg_replicate_means = {}
rg_replicate_stds = {}
...
for system in ["wt", "sdt3", "sdt5", "sdt8"]:
    ...
    for rep in [1, 2, 3, 4]:
        ...
        stds.append(np.std(clean, ddof=1))
    rg_replicate_means[system] = np.asarray(means, dtype=float)
    rg_replicate_stds[system] = np.asarray(stds, dtype=float)

base_y = 2.300
step_y = 0.010
comparisons = [("sdt3", 1), ("sdt5", 2), ("sdt8", 3)]

for system, idx in comparisons:
    # Welch t-test on replicate means
    ...
    rg_stats_rows.append({
        ...
        "filter_rule": "replicate-level means; frame-wise values shown only for visualization",
    })
    ...
    ax.text((x1 + x2) / 2, y + 0.001, significance_text(p_value), ha="center", va="bottom", fontsize=9)

fig.subplots_adjust(left=0.12, right=0.98, bottom=0.20, top=0.96)
fig.savefig(OUTPUT_DIR / "TOGT_SX4_Rg_violin.png", dpi=DPI, bbox_inches="tight")
plt.close(fig)
...
for center, system in zip(sasa_system_centers, ["wt", "sdt3", "sdt5", "sdt8"]):
    ax.text(
        center, -0.06,
        system,
...
fig.subplots_adjust(left=0.12, right=0.98, bottom=0.20, top=0.96)
fig.savefig(OUTPUT_DIR / "TOGT_SX5_total_SASA_violin_no_significance.png", dpi=DPI, bbox_inches="tight")
...
for system, idx in comparisons:
    ...
    sasa_stats_rows.append({
        ...
        "p_value": float(p_value),
        "significance": significance_text(p_value),
        "filter_rule": "values >= 195 only; frame-wise values plotted; significance tested on replicate-level means",
    })
...
fig.subplots_adjust(left=0.12, right=0.98, bottom=0.20, top=0.96)
fig.savefig(OUTPUT_DIR / "TOGT_SX5_total_SASA_violin.png", dpi=DPI, bbox_inches="tight")
plt.close(fig)

pd.DataFrame(rg_stats_rows).to_csv(OUTPUT_DIR / "TOGT_SX4_Rg_stats.csv", index=False)
pd.DataFrame(sasa_stats_rows).to_csv(OUTPUT_DIR / "TOGT_SX5_total_SASA_stats.csv", index=False)
...
avg_colors = np.where(highlight_mask, "#B23636", "#E6A0A0")
...
fig.savefig(OUTPUT_DIR / "TOGT_SX6_SX7_MMPBSA_decomposition.png", dpi=DPI, bbox_inches="tight")
plt.close(fig)
...
residue_energy_export.to_csv(
    OUTPUT_DIR / "TOGT_SX6_SX7_MMPBSA_residue_energy_details.csv",
    index=False,
    encoding="utf-8-sig",
)
...
summary_df.to_csv(
    OUTPUT_DIR / "TOGT_SX6_SX7_MMPBSA_energy_ratio_summary.csv",
    index=False,
    encoding="utf-8-sig",
)
...
sx9_df.to_csv(
    OUTPUT_DIR / "TOGT_SX9_distance_vs_dGbind_plot_data.csv",
    index=False,
    encoding="utf-8-sig",
)
...
def plot_sx9_distance_vs_dgbind(data, output_path, show_text=False):
    ...
    ax.axhspan(-1.40, 0, xmin=0, xmax=1, color="#FCFCFC", zorder=0)
    ax.axhspan(0, 0.25, xmin=0, xmax=1, color="#F7F7F7", zorder=0)

    sx9_groups = [
        ("Fixed", data.loc[data["is_conserved"] | data["is_first_shell"] | data["is_second_shell"]], SDT5_COLOR, "s"),
...
plot_sx9_distance_vs_dgbind(
    sx9_df,
    OUTPUT_DIR / "TOGT_SX9_distance_vs_dGbind_all.png",
    show_text=False,
)

plot_sx9_distance_vs_dgbind(
    sx9_df,
    OUTPUT_DIR / "TOGT_SX9_distance_vs_dGbind_all_with_text.png",
    show_text=True,
)

plot_sx9_distance_vs_dgbind(
    sx9_df.loc[sx9_df["count_value"] >= 4].copy(),
    OUTPUT_DIR / "TOGT_SX9_distance_vs_dGbind_count_gt_4.png",
    show_text=False,
)

plot_sx9_distance_vs_dgbind(
    sx9_df.loc[sx9_df["count_value"] >= 4].copy(),
    OUTPUT_DIR / "TOGT_SX9_distance_vs_dGbind_count_gt_4_with_text.png",
    show_text=True,
)
...
sx10_base_df.to_csv(
    OUTPUT_DIR / "TOGT_SX10_distance_vs_dataset_dGbind_plot_data.csv",
    index=False,
    encoding="utf-8-sig",
)
...
def plot_sx10_distance_vs_dataset_dgbind(data, output_path, fixed_color, show_text=False):
    fig, ax = plt.subplots(figsize=FIG_SCATTER)
    plot_data = data.copy()
    low_outlier_mask = plot_data["avg_dataset_value"] < -1.40
...
for system in ["wt", "sdt3", "sdt5", "sdt8"]:
    ...
    plot_sx10_distance_vs_dataset_dgbind(
        system_sx10_df,
        OUTPUT_DIR / f"TOGT_SX10_distance_vs_dataset_dGbind_{system}.png",
        SYSTEM_COLORS[system],
        show_text=False,
    )
    plot_sx10_distance_vs_dataset_dgbind(
        system_sx10_df,
        OUTPUT_DIR / f"TOGT_SX10_distance_vs_dataset_dGbind_{system}_with_text.png",
        SYSTEM_COLORS[system],
        show_text=True,
    )
